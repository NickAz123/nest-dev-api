...
export class GearRepository {
    ...
    async findById(id: number, client?: PoolClient): Promise<Gear | undefined> {
        const queryClient = client || this.pool;

        const result = await queryClient.query<Gear>(
            "SELECT g.*, gh.* FROM gear g LEFT JOIN gear_health gh on gh.gear_id = g.id WHERE g.is_deleted = FALSE AND g.id = $1 LIMIT 1",
            [id],
        );
    ...
    async findByUserId(userId: number): Promise<Gear[]> {
        const result = await this.pool.query<Gear>(
            "SELECT g.*, gh.* FROM gear g LEFT JOIN gear_health gh on gh.gear_id = g.id WHERE g.is_deleted = FALSE AND user_id = $1",
            [userId],
        );
        return result.rows;
    }
    ...
    async update(id: number, fields: UpdateGearDto): Promise<Gear | undefined> {
        ...
        return this.withTransaction(async (client) => {
            // Update gear
            const result = await client.query<Gear>(
                `UPDATE gear
